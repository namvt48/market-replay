...
Use the self-hosted **Roboto Variable** face for every interface and chart-canvas label, with the platform UI stack only as fallback. JetBrains Mono Variable is reserved for timestamps, prices, P&L, shortcuts, and stable numeric fields. The role scale is fixed and intentionally compact: **11/16px metadata**, **12/18px body**, **13/20px controls**, and **14/20px titles/symbols**. Avoid one-off 9px and 10px labels; hierarchy comes from role, weight, tone, and spacing rather than shrinking important information. Body uses 400, interactive controls 500, and headings 600; 700 is reserved for evaluation outcomes or similarly exceptional emphasis.
...
- **Operations panel:** fixed desktop rail and scrollable mobile tab surface; it never overlays the chart persistently. Evaluation is a first-class tab alongside Trade, Symbols, Journal, and Sessions.
- **Replay sessions:** a replay session is created only after a start bar is committed, never during workspace boot. The Sessions tab is a flat registry with a stable six-character display hash, exactly one `ACTIVE` session, resumable `PAUSED` sessions, terminal `STOPPED` sessions, core equity/performance statistics, and the closed-trade journal. Selecting a row only inspects it; Resume is explicit and checkpoints the previously active session first. Active sessions are checkpointed to `PAUSED` on exit or reload. Deleting an inactive session requires confirmation and removes its private trades and replay-scoped drawings. Timeline Play/Pause controls playback only and never change the session lifecycle. Trading actions and journal writes are accepted only for the active replay session (or a live evaluation). Session metrics stay inside the Sessions tab; the bottom replay dock remains transport-only.
...
- **Chart annotation:** blue default line treatment; replay and analysis scope is communicated in the toolbar and persisted as separate buckets. Drawing tools are one-shot. Selecting a drawing freezes chart navigation and opens a compact property inspector for geometry, stroke, fill, opacity, extension, and attached text.
- **Drawing rail and favorites:** the drawing rail stays fixed to the chart's left edge and remains visible while market data loads. Starred drawing tools persist locally and appear in a compact floating bar that can be dragged within the chart; its last position is restored and clamped when the chart resizes. Destructive actions remain behind an explicit selected/all choice. On narrow screens its inspector becomes a chart-contained bottom sheet.
