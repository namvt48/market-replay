...
- **Trade history table:** Journal and Evaluation use the same semantic, fixed-layout table ordered newest first. The four stable columns are Trade, Time, MFE/MAE, and P&L/R; trade dates always include the year, numeric cells use tabular mono figures, direction and result pair color with text/sign, and loading or empty histories preserve the same section frame.
