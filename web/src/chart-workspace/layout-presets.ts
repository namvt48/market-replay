...
import { DEFAULT_CHART_SYNC_FLAGS, type ChartPaneState, type ChartWorkspaceState, type LayoutNode, type LayoutPreset, type SplitOrientation } from './types'
...
function pane(id: string, timeframe: Timeframe, settings: ChartPaneSettings, symbol: string | null): ChartPaneState {
...
export function createLayoutPreset(preset: LayoutPreset, timeframe: Timeframe = '1m', inherited: ChartPaneSettings = DEFAULT_CHART_PANE_SETTINGS, symbol: string | null = null): ChartWorkspaceState {
  ...
  const panes = Object.fromEntries(Array.from({ length: count }, (_, index) => {
    ...
    return [id, pane(id, timeframe, inherited, symbol)]
  }))
