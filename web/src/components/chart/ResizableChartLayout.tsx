...
function SplitLayout({ node, renderPane, onResize }: ResizableChartLayoutProps & { node: Extract<LayoutNode, { kind: 'split' }> }) {
  ...
  return (
    <div ref={containerRef} className={`flex h-full min-h-0 min-w-0 ${horizontal ? 'flex-row' : 'flex-col'}`}>
      ...
      <button type="button" role="separator" aria-label={`Resize ${node.orientation} chart split`} aria-orientation={horizontal ? 'vertical' : 'horizontal'} aria-valuemin={20} aria-valuemax={80} aria-valuenow={Math.round(node.ratio * 100)} onPointerDown={start} onPointerMove={move} onPointerUp={stop} onPointerCancel={stop} onKeyDown={keyboard} style={horizontal ? { width: CHART_SPLIT_SEPARATOR_SIZE_PX, cursor: 'col-resize' } : { height: CHART_SPLIT_SEPARATOR_SIZE_PX, cursor: 'row-resize' }} className={`relative z-50 shrink-0 touch-none bg-line hover:bg-active focus-visible:bg-active ${horizontal ? 'before:absolute before:-inset-x-5 before:inset-y-0' : 'before:absolute before:inset-x-0 before:-inset-y-5'}`} />
