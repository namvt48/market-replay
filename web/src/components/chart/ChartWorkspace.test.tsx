...
vi.mock('./ChartTile', () => ({
  ChartTile: ({ pane, active, onActivate }: { pane: { id: string; timeframe: string; settings: { appearance: { backgroundColor: string } } }; active: boolean; onActivate: () => void }) => (
    <button type="button" data-testid="chart-tile" data-pane={pane.id} data-active={active} data-background={pane.settings.appearance.backgroundColor} onClick={onActivate}>{pane.id} {pane.timeframe}</button>
  ),
}))
