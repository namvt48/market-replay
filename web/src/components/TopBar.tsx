...
export function TopBar({ layoutMenuRequest = 0, onOpenShortcuts = () => undefined }: TopBarProps): ReactElement {
  ...
  return (
    <header className="workspace-topbar flex h-11 shrink-0 items-stretch border-b border-line bg-[#101114]" aria-label="Workspace controls">
        ...
        <nav className="flex shrink-0 items-center rounded-control bg-surface-0 p-0.5" aria-label="Chart timeframe">
          {visibleTimeframes.map((timeframe) => (
            <button
              ...
              className="topbar-control h-7 min-w-9 rounded-[3px] px-2 text-ui-control font-medium text-muted transition-colors hover:text-ink aria-pressed:bg-surface-3 aria-pressed:text-ink"
            >
              {timeframe}
            </button>
          ))}
