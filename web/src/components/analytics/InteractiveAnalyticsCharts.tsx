...
function linePoints(values: number[], width: number, height: number, inset: number, min: number, max: number): ChartPoint[] {
...
export function LineChart({ values, compact = false, ariaLabel, valueLabel = 'Value', valueFormatter = number, referenceLines = [], showPoints = false }: LineChartProps) {
  ...
  const points = linePoints(plottedValues, width, height, inset, min, max)
  const yForValue = (value: number): number => inset + (max - value) / Math.max(1, max - min) * (height - inset * 2)
    ...
    <TooltipSurface className={`w-full ${compact ? 'h-20' : 'h-64 min-w-[720px]'}`} inspectAtRatio={inspect}>
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" aria-label={`${ariaLabel}. Move the pointer across the chart to inspect each trade.`} tabIndex={0} onKeyDown={onKeyDown} onFocus={() => setKeyboardIndex(0)} onBlur={() => setKeyboardIndex(null)} className="h-full w-full outline-none focus-visible:ring-2 focus-visible:ring-active">
        ...
        {referenceLines.map((line) => {
          ...
          return <g key={`${line.label}-${line.value}`}><line x1="22" x2="978" y1={y} y2={y} stroke={color} strokeWidth="1.25" vectorEffect="non-scaling-stroke" /><text x="974" y={Math.max(12, y - 5)} textAnchor="end" fill={color} fontSize="11" fontWeight="600">{line.label}</text></g>
        })}
...
export function VerticalBars({ data, ariaLabel, tone = 'blue', valueSuffix = '' }: VerticalBarsProps) {
  ...
  return <TooltipSurface className="h-64 w-full min-w-[720px]"><svg viewBox="0 0 820 250" preserveAspectRatio="none" className="h-full w-full" role="img" aria-label={ariaLabel}>
    {[28, 80, 132, 184, 226].map((y) => <line key={y} x1="40" y1={y} x2="810" y2={y} stroke={colors.grid} strokeDasharray="7 7" vectorEffect="non-scaling-stroke" />)}
...
export function SplitBars({ data, ariaLabel, valueFormatter = signedMoney }: SplitBarsProps) {
  ...
  const step = 760 / data.length
  return <TooltipSurface className="h-64 w-full min-w-[720px]"><svg viewBox="0 0 820 250" preserveAspectRatio="none" className="h-full w-full" role="img" aria-label={ariaLabel}>
    {[28, 80, 132, 184, 226].map((y) => <line key={y} x1="40" y1={y} x2="810" y2={y} stroke={colors.grid} strokeDasharray="7 7" vectorEffect="non-scaling-stroke" />)}<line x1="40" y1="132" x2="810" y2="132" stroke={colors.axis} vectorEffect="non-scaling-stroke" />
...
export function FrequencyChart({ labels, values, ariaLabel }: FrequencyChartProps) {
  ...
  return <TooltipSurface className="h-44 w-full"><svg viewBox="0 0 260 170" className="h-full w-full" role="img" aria-label={ariaLabel}>
    {[15, 55, 95, 135].map((y) => <line key={y} x1="25" x2="250" y1={y} y2={y} stroke={colors.grid} strokeDasharray="6 6" />)}
